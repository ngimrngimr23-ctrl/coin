    ...
    while True:
        try:
            ...
            for pair in common:
                ...
                if htx_leg == "withdraw":
                    htx_transfer_label = "🔓 вывод" if htx_ok else "🔒 ВЫВОД ЗАКРЫТ"
                else:
                    htx_transfer_label = "🔓 ввод" if htx_ok else "🔒 ВВОД ЗАКРЫТ"
                if not htx_known:
                    htx_transfer_label = "❔ статус неизвестен"
                ...
                if buy_qty is not None and sell_qty is not None:
                    ...
                    depth_line += f" → прокрутить ~{_fmt_qty(tradable)} (~{fmt_money(tradable * buy_price)}$)"
                ...
                lines = [
                    ...
                    "",
                    f"🚚 Перевод: HTX {htx_transfer_label} · MEXC не проверяется (нет API-ключа)",
                ]
                if fee_line:
