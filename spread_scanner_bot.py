...
async def fetch_huobi_data():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://api.huobi.pro/market/tickers", headers=HTX_HEADERS, timeout=15) as resp:
                if resp.status != 200:
...
async def get_htx_transfer_status():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://api.huobi.pro/v2/reference/currencies", headers=HTX_HEADERS, timeout=20) as resp:
                if resp.status != 200:
                    body = await resp.text()
...
async def get_htx_depth(symbol_lower):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=HTX_HEADERS, timeout=8) as resp:
                if resp.status != 200:
                    return None, None
                data = await resp.json()
    ...
    while True:
        try:
            ...
            for pair in common:
                ...
                # У направления MEXC->HTX может быть свой порог (/spm); если не
                # задан (0) — используется общий spread_percent, как и для
                ...
                if spread_mexc_to_htx >= mexc_to_htx_threshold:
                    candidates.append(("MEXC", "HTX", spread_mexc_to_htx, m["ask"], h["bid"]))
                if spread_htx_to_mexc >= htx_to_mexc_threshold:
                    candidates.append(("HTX", "MEXC", spread_htx_to_mexc, h["ask"], m["bid"]))
                ...
                buy_ex, sell_ex, best_spread, buy_price, sell_price = max(candidates, key=lambda c: c[2])
